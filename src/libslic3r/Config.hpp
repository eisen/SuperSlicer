...
private:
    friend class cereal::access;
    template<class Archive> void serialize(Archive & ar) { ar(this->phony); ar(this->value); ar(this->percent); }
};
...
public:
    ...
    inline static std::stringstream config_hpp_buff = std::stringstream();
private:
	friend class cereal::access;
    template<class Archive> void serialize(Archive& ar) { 
        std::cout << "bef:'";
        for(int i=0;i< config_hpp_buff.size();i++)
            std::cout << (i%4==0?" ":":") <<(uint16_t)config_hpp_buff[i];
        std::cout<<"'\n"
    ar(cereal::base_class<ConfigOptionInt>(this));
        std::cout << "aft:'";
        for (int i = 0; i < config_hpp_buff.size(); i++)
            std::cout << (i % 4 == 0 ? " " : ":") << (uint16_t)config_hpp_buff[i];
        std::cout << "'\n"
...
public:
    ...
    template<class Archive> ConfigOption* load_option_from_archive(Archive &archive) const {
    	if (this->nullable) {
            ...
            case coEnum: { auto opt = new ConfigOptionEnumGeneric(this->enum_keys_map); archive(*opt); std::cout << "load from archive: " << opt->getInt() << "\n"; return opt; }
    ...
    template<class Archive> ConfigOption* save_option_to_archive(Archive &archive, const ConfigOption *opt) const {
    	if (this->nullable) {
		    ...
		    case coEnum:            archive(*static_cast<const ConfigOptionEnumGeneric*>(opt)); std::cout << "save to archive: " << opt->getInt() << "\n";	break;
